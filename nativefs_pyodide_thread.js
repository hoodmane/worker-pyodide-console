function addNativeFS(pyodide, mainThreadNativeFSHelpers) {
  ...
  const nativeFSHelpers = {
    ...
    truncate(fileHandle, length) {
      return mainThreadNativeFSHelpers.truncate(fileHandle, length).syncify();
    },
    touch(fileHandle) {
  ...
  const NATIVEFS = {
    ...
    node_ops: {
      ...
      setattr: function (node, attr) {
        ...
        if (attr.timestamp !== undefined && attr.timestamp > node.timestamp) {
          ...
          node.timestamp = new Date(nativeFSHelpers.touch(node.handle));
        }
      ...
      lookup: function (parent, name) {
        ...
        if (isDir) {
          ...
          timestamp = new Date(nativeFSHelpers.getFileTimestamp(handle));
        }
    ...
    stream_ops: {
      ...
      write: function (stream, buffer, offset, length, position, canOwn) {
        ...
        try {
          nativeFSHelpers.writeToFile(stream.node.handle, position, data);
        } catch (e) {
          throw new FS.ErrnoError(63 /* EPERM */);
        }
